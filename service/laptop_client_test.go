...
func TestClientCreateLaptop(t *testing.T) {
	...
	// check that the laptop is the same as the one we send
...
func TestClientRateLaptop(t *testing.T) {
	...
	for idx := 0; ; idx++ {
		...
		require.Equal(t, res.GetRatedCount(), uint32(idx+1))
...
func startTestLaptopServer(t *testing.T, laptopStore service.LaptopStore, imageStore service.ImageStore, ratingStore service.RatingStore) string {
	...
	go grpcServer.Serve(listener) // blocking call we need to run it in separate goroutine
...
func requireSameLaptop(t *testing.T, laptop, other *pb.Laptop) {
	// --> require.Equal(t, laptop, other) // this will produce error because there is special field internaly by grpc to serialize their object

	// the easy ways to compare the object is serialize the object to json
	laptopJson, err := serializer.ProtobufToJSON(laptop)
	require.NoError(t, err)

	otherJson, err := serializer.ProtobufToJSON(other)
	require.NoError(t, err)
